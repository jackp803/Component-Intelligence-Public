...
public sealed class DrawingPlanEditService
{
    ...
    {
        var pages = plan.Pages.ToArray(); var index = Array.FindIndex(pages, page => page.PageId == pageId);
        if (index < 0) throw new InvalidOperationException("Page not found.");
        pages[index] = pages[index] with { OrderState = state };
        return DrawingPlanJson.Rehash(plan with { Pages = pages });
    }

    public DrawingPlanDocument MoveRouteSegment(DrawingPlanDocument plan, string routeId, int segmentIndex, long delta) => UpdateRoute(plan, routeId, route =>
    {
    ...
    {
        EnsureEditable(route); if (segmentIndex < 0 || segmentIndex >= route.Points.Count - 1) throw new InvalidOperationException("Route segment index is invalid.");
    ...
    {
        EnsureEditable(route); if (pointIndex <= 0 || pointIndex >= route.Points.Count - 1) throw new InvalidOperationException("Only interior bend points can be deleted.");
    ...
    public DrawingPlanDocument AlignPlacements(DrawingPlanDocument plan, IReadOnlyList<string> representationIds, DrawingAlignment alignment)
    {
        ...
        {
            DrawingAlignment.Left => selected.Min(x => x.X), DrawingAlignment.Right => selected.Max(x => x.X + x.Width), DrawingAlignment.Top => selected.Min(x => x.Y), DrawingAlignment.Bottom => selected.Max(x => x.Y + x.Height),
    ...
    {
        if (!plan.Pages.Any(x => x.PageId == pageId)) throw new InvalidOperationException("Page not found."); var groupIds = plan.Groups.Where(x => x.PageId == pageId).Select(x => x.GroupId).ToHashSet(StringComparer.Ordinal);
    ...
    {
        DrawingAlignment.Left => p with { X = target, State = DrawingPlanControlState.Manual }, DrawingAlignment.Right => p with { X = target - p.Width, State = DrawingPlanControlState.Manual }, DrawingAlignment.Top => p with { Y = target, State = DrawingPlanControlState.Manual }, DrawingAlignment.Bottom => p with { Y = target - p.Height, State = DrawingPlanControlState.Manual }, DrawingAlignment.HorizontalCenter => p with { X = target - p.Width / 2, State = DrawingPlanControlState.Manual }, DrawingAlignment.VerticalCenter => p with { Y = target - p.Height / 2, State = DrawingPlanControlState.Manual }, _ => p
    };

    private static IReadOnlyList<DrawingPlacement> SelectPlacements(DrawingPlanDocument plan, IReadOnlyList<string> ids)
    {
        if (ids.Count == 0) throw new InvalidOperationException("At least one placement is required."); var set = ids.ToHashSet(StringComparer.Ordinal); var selected = plan.Placements.Where(x => set.Contains(x.RepresentationId)).ToList();
        if (selected.Count != set.Count) throw new InvalidOperationException("Placement selection contains unknown ID."); if (selected.Any(x => x.State == DrawingPlanControlState.Locked)) throw new InvalidOperationException("Locked placement cannot be edited."); return selected;
    }
    ...
    {
        var items = plan.Routes.ToArray(); var index = Array.FindIndex(items, r => r.RouteId == id); if (index < 0) throw new InvalidOperationException("Route not found."); items[index] = update(items[index]); return DrawingPlanJson.Rehash(plan with { Routes = items });
    }
}
