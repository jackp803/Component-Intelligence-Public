...
public static class DrawingPlanJson
{
    ...
    public static DrawingPlanDocument Deserialize(string json)
    {
        ...
        var plan = JsonSerializer.Deserialize<DrawingPlanDocument>(json, Options)
            ?? throw new InvalidDataException("Drawing Plan is empty.");
        Validate(plan, requireHash: true);
        var node = JsonSerializer.SerializeToNode(plan with { DrawingPlanHash = null }, Options)!.AsObject();
        var expected = Sha256(CanonicalText(Canonicalize(node, null)));
        if (!string.Equals(expected, plan.DrawingPlanHash, StringComparison.Ordinal))
            throw new InvalidDataException("drawingPlanHash mismatch.");
    ...
    private static void Validate(DrawingPlanDocument plan, bool requireHash)
    {
        ...
        RequireHash(plan.SourcePlanningInputHash, nameof(plan.SourcePlanningInputHash));
        RequireHash(plan.SourcePagePlanHash, nameof(plan.SourcePagePlanHash));
        if (requireHash) RequireHash(plan.DrawingPlanHash, nameof(plan.DrawingPlanHash));
        EnsureUnique(plan.Pages, x => x.PageId, "pageId"); EnsureUnique(plan.Groups, x => x.GroupId, "groupId");
        EnsureUnique(plan.Placements, x => x.RepresentationId, "representationId"); EnsureUnique(plan.Routes, x => x.RouteId, "routeId");
        var pageIds = plan.Pages.Select(x => x.PageId).ToHashSet(StringComparer.Ordinal);
        var groupIds = plan.Groups.Select(x => x.GroupId).ToHashSet(StringComparer.Ordinal);
        foreach (var placement in plan.Placements)
        {
            ...
            if (placement.AllowedRotations.Count == 0 || placement.AllowedRotations.Any(x => x is not (0 or 90 or 180 or 270)) || !placement.AllowedRotations.Contains(placement.RotationDegrees))
                throw new InvalidDataException("Placement rotation is not allowed.");
        }
        foreach (var route in plan.Routes)
        {
            ...
            for (var i = 1; i < route.Points.Count; i++)
                if (route.Points[i - 1].X != route.Points[i].X && route.Points[i - 1].Y != route.Points[i].Y) throw new InvalidDataException("Route geometry must be orthogonal.");
        }
    }

    private static void RequireHash(string? value, string name)
    {
        if (string.IsNullOrWhiteSpace(value) || !HashPattern.IsMatch(value)) throw new InvalidDataException($"{name} must be uppercase SHA-256.");
    }
    ...
    private static JsonNode? Canonicalize(JsonNode? node, string? parentKey)
    {
        if (node is JsonObject obj)
        {
            ...
            foreach (var pair in obj.OrderBy(x => x.Key, StringComparer.Ordinal)) result[pair.Key] = Canonicalize(pair.Value?.DeepClone(), pair.Key);
            return result;
        }
    ...
    {
        var options = new JsonSerializerOptions(JsonSerializerDefaults.Web) { PropertyNamingPolicy = JsonNamingPolicy.CamelCase, PropertyNameCaseInsensitive = false, WriteIndented = false };
