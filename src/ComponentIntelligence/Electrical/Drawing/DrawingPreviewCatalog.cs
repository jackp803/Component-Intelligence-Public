...
public static class DrawingPreviewCatalog
{
    public static IReadOnlyList<DrawingRepresentationDecision> Build(DrawingPlanningInput input)
    {
        ...
        var order = DrawingContactDisplayOrder.Read(input);
        foreach (var item in input.HeavyDutyConnectors)
        {
            ...
            var contacts = item.ContactIds.OrderBy(id => order.GetValueOrDefault(id, int.MaxValue)).ThenBy(id => id, StringComparer.Ordinal).ToArray();
            ...
            for (var index = 0; index < chunks.Length; index++)
                result.Add(original with
                {
                    ...
                    PortBindings = original.PortBindings.Where(b => chunks[index].Contains(b.EngineeringEndpointId, StringComparer.Ordinal)
                        || (index == 0 && !contacts.Contains(b.EngineeringEndpointId, StringComparer.Ordinal)))
                        .OrderBy(b => order.GetValueOrDefault(b.EngineeringEndpointId, int.MaxValue)).ThenBy(b => b.EngineeringEndpointId, StringComparer.Ordinal).ToArray()
                });
        }
